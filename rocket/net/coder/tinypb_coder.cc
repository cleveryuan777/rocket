    ...
    void TinyPBCoder::decode(std::vector<AbstractProtocol::s_ptr> &out_messages, TcpBuffer::s_ptr buffer)
    {
        ...
        while (1)
        {
            ...
            if (parse_success)
            {
                ...
                DEBUGLOG("parse m_msg_id_len = %d", message->m_msg_id_len);
                ...
                DEBUGLOG("parse m_msg_id = %s", message->m_msg_id.c_str());
                ...
                DEBUGLOG("parse m_msg_id = %s", message->m_method_name.c_str());
