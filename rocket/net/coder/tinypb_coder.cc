...
namespace rocket
{
    ...
    void TinyPBCoder::decode(std::vector<AbstractProtocol::s_ptr> &out_messages, TcpBuffer::s_ptr buffer)
    {
        ...
        while (1)
        {
            // 遍历buffer，知道PB_START, 找到之后，解析出整包的长度。然后得到结束符的位置，判断是否为PB_END
            ...
            if (parse_success)
            {
                ...
                DEBUGLOG("parse m_req_id_len = %d", message->m_req_id_len);
                ...
                DEBUGLOG("parse m_req_id = %s", message->m_req_id.c_str());
                ...
                DEBUGLOG("parse m_req_id = %s", message->m_method_name.c_str());
